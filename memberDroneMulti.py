...
def arm():
    ...
    while not vehicle.is_armable:
        if not armContinue:
            print(str(now.time()) +  f": Arm canceled.\n")
...
def takeoff(target_altitude):
    ...
    while True:
        if not takeoffContinue:
            print(str(now.time()) +  f": Take off canceled.\n")
...
def goto_position(vehicle, latitude, longitude, altitude):
    ...
    while True:
        if not gotoDirectionContinue:
            print(str(now.time()) +  f": Directional task canceled.\n")
    ...
    try:
        ...
        if msgArray[0] == "REPORTGPS":
            ...
            if gotoDirectionContinue:
                print(str(now.time()) +  f": Directional task recieved but another task is already running!\n")
                raise
            ...
            print(str(now.time()) +  f": Directional task stop request recieved from {sock.getpeername()}\n")
            gotoDirectionContinue = False
        elif msgArray[0] == "ARM":
            print(str(now.time()) +  f": Arm request recieved from {sock.getpeername()}\n")
            if armContinue:
                print(str(now.time()) +  f": Arm request recieved but another arm request is already running!\n")
                raise
            armThread = threading.Thread(target=arm)
            armThread.start()
        elif msgArray[0] == "STOPARM":
            print(str(now.time()) +  f": Arm stop request recieved from {sock.getpeername()}\n")
            armContinue = False
            
        elif msgArray[0] == "TAKEOFF":
            print(str(now.time()) +  f": Takeoff request recieved from {sock.getpeername()} for altitude {msgArray[1]} m\n")
            if takeoffContinue:
                print(str(now.time()) +  f": Takeoff recieved but another takeoff task is already running!\n")
                raise
            ...
            print(str(now.time()) +  f": Take off stop request recieved from {sock.getpeername()}\n")
            takeoffContinue = False 
